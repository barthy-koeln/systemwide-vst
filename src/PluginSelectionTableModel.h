...
public:
    ...
    void paintRowBackground (
      ...
      bool rowIsSelected
        ...
        g.fillAll(rowIsSelected ? this->lookAndFeel.primaryDark : this->lookAndFeel.backdrop);
    }
    ...
    [[nodiscard]] juce::String getCellText (bool isBlacklisted, int row, int columnId) const {
        ...
        switch (columnId) {
            case nameCol:
                return desc.name;
            case typeCol:
                return desc.pluginFormatName;
            case categoryCol:
                return desc.category.isNotEmpty() ? desc.category : "-";
            case manufacturerCol:
                return desc.manufacturerName;
            case descCol:
                return getPluginDescription(desc);
            default:
                jassertfalse;
                break;
        }
    ...
    void paintCell (juce::Graphics &g, int row, int column, int width, int height, bool /*rowIsSelected*/) override {
        ...
        g.setColour(
          ...
          (isActive ? this->lookAndFeel.backdrop : this->pluginListComponent.findColour(juce::ListBox::textColourId))
        );
    ...
    void cellClicked (int row, int column, const juce::MouseEvent &e) override {
        TableListBoxModel::cellClicked(row, column, e);
    ...
    void cellDoubleClicked (int row, int column, const juce::MouseEvent &e) override {
        TableListBoxModel::cellDoubleClicked(row, column, e);
    ...
    void sortOrderChanged (int newSortColumnId, bool isForwards) override {
        switch (newSortColumnId) {
            case nameCol:
                this->list.sort(juce::KnownPluginList::sortAlphabetically, isForwards);
                break;
            case typeCol:
                this->list.sort(juce::KnownPluginList::sortByFormat, isForwards);
                break;
            case categoryCol:
                this->list.sort(juce::KnownPluginList::sortByCategory, isForwards);
                break;
            case manufacturerCol:
                this->list.sort(juce::KnownPluginList::sortByManufacturer, isForwards);
                break;
            case descCol:
                break;
